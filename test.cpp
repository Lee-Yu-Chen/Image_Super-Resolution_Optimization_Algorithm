...
int main(int argc, char** argv)
{
    ...
    */
    ...
    Pixels up_pixel(up);
    Pixels up_pixel2(up2);

    Quantum* up_pix = up_pixel.get(0, 0, width*ups, height*ups);
    Quantum* up_pix2 = up_pixel2.get(0, 0, w2*ups, h2*ups);



    // generate an up-sampled image by combining up.png and up2.png 
    Image up_combined(Geometry(width* ups, height* ups), Color("#000000"));
    Pixels up_p(up_combined);
    Quantum* up_q = up_p.get(0, 0, width * ups, height * ups);

    cout << "\n\nCombining up_sampled image ...\n\n";

    I = 0;
    for (int i = 0; i <= height*ups - 1; i++) {
        for (int j = 0; j <= width*ups - 1; j++) {

            *(up_q + I) = (*(up_pix + I) + *(up_pix2 + I)) / 2;
            *(up_q + I + 1) = (*(up_pix + I + 1) + *(up_pix2 + I + 1)) / 2;
            *(up_q + I + 2) = (*(up_pix + I + 2) + *(up_pix2 + I + 2)) / 2;
            I += 3;
    ...
    up_p.sync();
    up_combined.write("C:/Users/ACER/Desktop/up_sampled_combined.png");
